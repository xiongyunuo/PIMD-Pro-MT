...
void XCreateWorker(int index, int N, int P, XNum omgP, XNum L, XParticle *particles, XNum *ENkCache2, XNum *ENkCache3, XNum **ENkCache, XParticle *particles2, int NN, XNum *VBCache, XNum *ForceVBCache, int NT, XNum vi, XNum beta, XParticle center, XNum omg0, XNum g, XNum s, XNum h) {
...
void XSimulation::SpamWorkers() {
  ...
  XGo6.store(false);
  XIni.store(true);
  XCount.store(thread_count);
  int N2 = N/thread_count;
  if (N % thread_count != 0)
    ++N2;
  XWorkers = new XSimulationWorker*[thread_count];
  int i;
  for (i = 0; i < thread_count; ++i) {
    int N3 = N2;
    if (i == thread_count-1)
      N3 = N-N2*i;
    std::thread t(XCreateWorker, i, N3, P, omgP, L, &particles[i*N2*P], &ENkCache2[i*N2], ENkCache2, ENkCache, particles, i*N2, VBCache, &ForceVBCache[D*P*i*N2], N, vi, beta, center, omg0, g, s, h);
    t.detach();
  }
}
